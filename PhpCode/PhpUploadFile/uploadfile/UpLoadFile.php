...
class Upload{
    ...
    public $fileFormat = array('gif','jpg','jpeg','png','application/octet-stream');// 文件格式&MIME限定
...
// public function __construct($savePath =  "/tem/uploads", $fileFormat='',$maxSize = 0, $overwrite = 0) {
public function __construct($savePath =  "./file/uploads", $fileFormat='',$maxSize = 0, $overwrite = 0) {
...
// @param $changeName 是否更改文件名
    public function save($fileInput,$changeName = 1){
...
// @param $fileArray 文件信息数组
    public function copyfile($fileArray){
        ...
        $this->returninfo['fullName'] = $this->savePath . $this->saveName;
        ...
        if (!file_exists($this->savePath)) {
            // 创建多级目录
            $mkdirPathStatus = mkdir($this->savePath, 0777, true);
            if (!$mkdirPathStatus) {
                $this->errno = 23;
                return false;
            }
